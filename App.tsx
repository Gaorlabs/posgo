           ...
           <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex gap-2">
                 ...
                 <div className="relative">
                    ...
                    <label htmlFor="import-excel" className="px-4 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-bold flex items-center gap-2 cursor-pointer hover:bg-slate-50 transition-colors shadow-sm">
              ...
              <div className="overflow-y-auto flex-1 p-2 custom-scrollbar">
                 ...
                 {inventoryTab === 'ALL' ? (
                     ...
                     <div className="p-4 grid grid-cols-1 gap-4">
                         <div className="bg-orange-50 p-6 rounded-2xl border border-orange-100 flex items-start gap-4 mb-4">
                             <div className="p-3 bg-white rounded-full shadow-sm"><Flame className="w-6 h-6 text-orange-500"/></div>
                             <div>
                                 <h3 className="font-bold text-orange-800 text-lg">Sugerencias de Reposición</h3>
                                 <p className="text-orange-700/80 text-sm">Esta lista muestra productos con <b>Stock Bajo (≤ 5)</b> ordenados por <b>Velocidad de Venta</b>. Los primeros productos son los que más vendes y urge reponer.</p>
                             </div>
                         </div>
                         ...
                         {replenishmentData.length === 0 ? (
                             <div className="text-center p-12 text-slate-400">
                                 ...
                                 <p className="font-bold text-lg text-slate-600">¡Todo en orden!</p>
                                 <p>No tienes productos con stock crítico en este momento.</p>
                             </div>
                         ) : (
                             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                 {replenishmentData.map((p, idx) => (
                                     <div key={p.id} className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
                                         {/* Rank Badge */}
                                         <div className="absolute top-0 left-0 bg-slate-100 text-slate-500 px-3 py-1 rounded-br-xl text-xs font-black">
                                             #{idx + 1} Prioridad
                                         </div>
                                         ...
                                         <div className="absolute top-4 right-4">
                                             {p.salesVelocity > 10 ? (
                                                 <div className="flex items-center gap-1 text-[10px] font-bold bg-red-100 text-red-600 px-2 py-1 rounded-full border border-red-200">
                                                     <Flame className="w-3 h-3"/> ALTA ROTACIÓN
                                                 </div>
                                             ) : (
                                                 <div className="flex items-center gap-1 text-[10px] font-bold bg-slate-100 text-slate-500 px-2 py-1 rounded-full">
                                         ...
                                         <div className="mt-6 mb-4">
                                             ...
                                             <p className="text-xs text-slate-400 uppercase font-bold">{p.category}</p>
                                         </div>
                                         ...
                                         <div className="flex items-end justify-between bg-slate-50 p-3 rounded-xl">
                                             <div>
                                                 ...
                                                 <p className="text-2xl font-black text-red-500">{p.stock} <span className="text-sm text-slate-400 font-medium">un.</span></p>
                                             </div>
                                             <div className="text-right">
                                                 <p className="text-[10px] uppercase font-bold text-slate-400">Ventas Hist.</p>
                                                 <p className="text-sm font-bold text-slate-700">{p.salesVelocity} un.</p>
                                             </div>
                                         </div>
                                         
                                         <button 
                                            onClick={() => {
                                                setPurchaseSearchTerm(p.name);
                                                setView(ViewState.PURCHASES);
                                                setPurchaseTab('NEW');
                                            }}
                                            className="w-full mt-4 py-2 bg-slate-800 text-white rounded-xl font-bold text-sm hover:bg-black transition-colors opacity-0 group-hover:opacity-100"
                                         >
                                             Reponer Ahora
                                         </button>
                                     </div>
